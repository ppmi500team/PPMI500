...
temp=meeko$demog[ meeko$demog$allpass & meeko$demog$DXSubAsyn=='ProdromalSporadicPositive', ]
for ( zz in 1:length(simsub))
  temp[,simsub[zz]]=psych::winsor( temp[,simsub[zz]], 0.01)
mdl1=lm( mform1 , data=temp )
summary(mdl1)
...
```{r interpsimg,echo=FALSE,eval=TRUE,message=FALSE,include=TRUE,cache=FALSE,fig.width=8,fig.height=8}
# myinterp=0
n2show=10
...
ggbarplot( myggdf, 'idp', 'value', fill='modality',  orientation = "horiz", sort.val ='asc' )
...
gghistogram( permdf, x = 'logp',
      ...
      xlab='-log_10(p) updrs total score from t1, rsfmri + dti')
```