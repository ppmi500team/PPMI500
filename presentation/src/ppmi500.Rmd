...
temp2=aggregate( qchuman_DTI ~ imageID, temp2, mostfreq ) 
...
rsfqc=add_vectors_with_na( as.numeric( ppmi500$qchuman_rsfMRI.x ), as.numeric( ppmi500$qchuman_rsfMRI.y ) )

xcl=c('winz','threshold','original')
nuisnames = unique( c( 
  getNamesFromDataframe( c("rsfMRI","122","snr"), ppmi500, exclusions=xcl),
  getNamesFromDataframe( c("rsfMRI","122","FD"), ppmi500, exclusions=xcl),
  getNamesFromDataframe( c("rsfMRI","122","minutes"), ppmi500, exclusions=xcl),
  getNamesFromDataframe( c("rsfMRI","122","despik"), ppmi500, exclusions=xcl ),
  getNamesFromDataframe( c("rsfMRI","122","evr"), ppmi500, exclusions=xcl)))
for ( n in nuisnames ) {
  nn=paste0(n,"_winz")
  nn=gsub("rsfMRI_fcnxpro122_","",nn)
  if ( length(grep("minutes",n) > 0 ) ) {
    ppmi500[,nn]=ppmi500[,n]
    ppmi500[ subtyper::fs(ppmi500[,nn] > 10) ,nn]=ppmi500[ subtyper::fs(ppmi500[,nn] > 10) ,nn]-5
  } else ppmi500[,nn]=psych::winsor( ppmi500[,n], 0.01 )
}
nuisnames=getNamesFromDataframe("_winz",ppmi500)
mform = paste( "rsfqc ~ ", paste( nuisnames, collapse='+') )
mdl=lm(   mform , data=ppmi500)
...
mform = paste( "cnxvar ~ joinedDX+commonAge+commonSex+ despiking_count_summary_winz+ssnr_mean_winz+FD_max_winz")
tempx=temp[temp$FD_max_winz<=1.5,]
...
print( modelplot( list( passAndFail=mdlF, passOnly=mdlP, AutoQC=mdlD, ManQC=mdlD2 ), 
  ...
  + ggtitle( paste("Models: All vs Pass vs AutoQC+DX + ManQC+DX (select covars)") )
  +    theme(
