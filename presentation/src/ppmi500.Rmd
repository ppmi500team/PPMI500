...
mform=paste0( "T1Hier_vol_mtg_sn_snc_LRAVGcit168~joinedDX+brainVolume+commonAge+commonSex+",paste(nuisnames,collapse='+'))
ppmi500sel=scale_variables_in_equation(ppmi500sel,mform)
...
print( modelplot( list(  
  ...
  AutoQC=mdlAuto, ManQC=mdlMan ), 
    conf_level = .95, coef_omit = "nterc|evr|LRR|spc")
    + ggtitle( paste("T1w Models: All vs Permissive vs AutoQC + ManQC \n (select covars shown) SNC volume") )
    +    theme(
            plot.title = element_text(size = 20),
            axis.title.x = element_text(size = 18),
            axis.title.y = element_text(size = 18), 
