...
xx=normativeSummary( ppmi500sel, wrow, 
  c("commonAge", "DXSubAsyn", 
    "T1Hier_vol_mtg_sn_snc_LRAVGdeep_cit168", 
    "T1Hier_vol_bn_gp_gpe_LRAVGdeep_cit168", 
    "T2Flair_flair_wmh_prior", 
    "T1Hier_vol_bn_str_pu_LRAVGcit168", slfasym, rsfvar, nmvar, 'updrs_totscore' ) )
# Sys.sleep(3)
# }
...
if ( ! ( all( c("qchuman_T1w","qchuman_NM") %in% colnames(ppmi500))) ) {
  ppmi500=temp
}
...
success_failure_data$modality=c("T1w",'FLAIR',"DTI","DTI","rsfMRI","rsfMRI","NM")
...
ppmi500$rsfqc=add_vectors_with_na( as.numeric( ppmi500$qchuman_rsfMRI.x ), as.numeric( ppmi500$qchuman_rsfMRI.y ) )
...
print( modelplot( list( passAndFail=mdlF, passOnly=mdlP, AutoQC=mdlD, ManQC=mdlD2 ), 
  conf_level = .95, coef_omit = "nterc|tsnr|despik|FD_mean|evr|LRR")
  + ggtitle( paste("rsfMRI Models: All vs Pass vs AutoQC+DX + ManQC+DX (select covars shown)") )
  +    theme(
...
ppmi500$dtiqc=add_vectors_with_na( as.numeric( ppmi500$qchuman_DTI.x ), as.numeric( ppmi500$qchuman_DTI.y ) )
slit = subtyper::fs(ppmi500$T1Hier_resnetGrade > 1.02)
...
for ( slfasym in  getNamesFromDataframe( c("DTI","orpus"), ppmi500 ) ) {
  ...
  mform = paste( "cnxvar ~ joinedDX+commonAge+commonSex+brainVolume+
  dti.ssnr_b0_mean_winz+  dti.FD_mean_winz+dti.FD_max_winz+dti.fa_evr_winz")
  ...
  print( modelplot( list( passAndFail=mdlF, passOnly=mdlP, AutoQC=mdlD, ManQC=mdlD2 ), 
    conf_level = .95, coef_omit = "nterc|snr|LRR|CNP|brainVolume")
...
xcl=c('winz','threshold','original','rsfMR','cnxc')
nuisnames = c("NM2DMT_NM_avg_signaltonoise","NM2DMT_NM_evr","NM2DMT_NM_sd","NM2DMT_NM_substantianigra_z_coordinate")
for ( n in nuisnames ) {
  nn=paste0(n,"_winz")
...
temp=ppmi500sel
temp$nmvar = temp$NM2DMT_NM_avg_substantianigra - temp$NM2DMT_NM_avg_refregion
temp$commonAge=as.numeric(scale(temp$commonAge))
...
mform = paste( "nmvar ~ joinedDX+commonAge+commonSex+brainVolume+", paste( nuisnames, collapse='+'))
tempx=temp[tempx$nm.sd_winz < 1,]
tempx=scale_variables_in_equation( tempx, mform)
mdlD=lm(   mform , data=tempx)
tempx=temp[temp$NM_QC_Ratings_BA==1,]
tempx=scale_variables_in_equation( tempx, mform)
...
mform=paste0( "T2Flair_flair_wmh_prior~joinedDX+brainVolume+commonAge+commonSex+",paste(nuisnames,collapse='+'))
...
print( modelplot( list(  AutoQC=mdlAuto, ManQC=mdlMan ), 
...
vois=c("T1Hier_vol_mtg_sn_snc_LRAVGcit168","T1Hier_thk_LRAVG_superior_parietaldktcortex",
  "T1Hier_vol_bn_gp_gpe_Asymdeep_cit168",
  "T1Hier_vol_bn_gp_gpi_Asymdeep_cit168","T1Hier_vol_LRAVG_dg.ca3mtl")
for ( voi in vois ) {
