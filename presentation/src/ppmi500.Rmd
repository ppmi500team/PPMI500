...
```{r simlr,echo=FALSE,eval=TRUE,fig.width=9,fig.height=3}
######
...
#      mats[[x]]=residuals( lm( data.matrix(mats[[x]]) ~ 1, data=blaster2[allnna,]))
...
for ( permxpt in 0:20 ) {
  ...
  # mform2=paste(bform,"*(",paste(simsub,collapse='+'),")")
    ...
    temp[,simsub[2]]=psych::winsor( temp[,simsub[2]], 0.01)
...
#    myanv2=anova(mdl1,mdl2)
    ...
    visreg::visreg( mdl1, simsub[1] , main=paste('T1', permstate,insight::format_p(myco['t1PC1','Pr(>|t|)'])) )
    visreg::visreg( mdl1, simsub[2] , main=paste('rsfMRI', permstate,insight::format_p(myco['rsPC1','Pr(>|t|)'])) )
    visreg::visreg( mdl1, simsub[3], main=paste('DTI', permstate,insight::format_p(myco['dtPC1','Pr(>|t|)'])) )
    nn=nrow(permdf)+1
